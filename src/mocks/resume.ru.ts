...
export const resumeRu: ResumeInterface = {
  ...
  summary: {
    text: [
      'Я люблю программировать и деньги, а еще больше — создавать проекты, которые приносят деньги. Для меня деньги — это главный критерий успешности продукта. Они показывают, насколько продукт востребован и полезен. Это самый простой и объективный способ понять, что твое решение действительно нужно людям и решает их реальные проблемы.',
      'За {experienceInYears} лет работы я участвовал в самых разных проектах, от криптовалютных платформ и NFT-рынков до систем искусственного интеллекта. Моя основная специализация — Node.js и TypeScript, а также разработка API и архитектура микросервисов.',
      'Помимо работы над Backend, я также занимаюсь Frontend-разработкой. Это позволяет мне создавать полноценные решения, не упуская важных аспектов взаимодействия между клиентской и серверной частями.',
      'Я открыт для работы и интересных предложений. Буду рад обсудить возможное сотрудничество!',
    ],
  },
  education: {
    items: [
